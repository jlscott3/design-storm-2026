// MOCK aluminium sulfate (alum) dose — ILLUSTRATIVE ONLY, NOT A REAL CALCULATION.
//
// We were given ONE unverified data point from Denver Water's operator Excel sheet
// (Cassidi/Jake Q&A, 2026-09-24): incoming TOC of ~3 mg/L targets "~11 of alum",
// with the unit noted as unconfirmed (grams vs mg/L). We treat the dose as a rate in
// mg/L, per normal practice.
//
...
//   base rate  = ALUM_PER_TOC * predicted_TOC     (11/3 ≈ 3.67 mg/L alum per mg/L TOC)
...
export const ALUM_PER_TOC = 11 / 3 // mg/L alum per mg/L TOC, from the one example
...
/**
 * Compute the mock alum dose from a forecast TOC and alkalinity.
 *
 ...
 */
export function mockAlumDose(tocValue, alkValue, alkWatch = 60) {
  if (tocValue == null || !Number.isFinite(tocValue)) return null

  const base = ALUM_PER_TOC * tocValue
